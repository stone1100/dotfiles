...
local blinkOk, blink = pcall(require, "blink.cmp")
if blinkOk then
	lsp.capabilities = blink.get_lsp_capabilities(capabilities)
end
local cmpOk, cmp = pcall(require, "cmp_nvim_lsp")
if cmpOk then
	lsp.capabilities = cmp.default_capabilities(capabilities)
end
